...
import {
  Upload, File, Download, RotateCcw, Wand2, Search, Check,
  ChevronDown, ChevronUp, Music, Video, Image, FileText,
  AlertCircle, CheckCircle, Info, CloudUpload, X, Clock
} from 'lucide-react';
...
export default function Convert() {
  ...
  const filteredFormats = Object.entries(supportedFormats).reduce((acc, [category, formats]) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4 md:p-8">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg md:text-xl text-gray-300 mb-6">Transform files effortlessly with Moon-Convert</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white/10 backdrop-blur-lg rounded-3xl p-6 shadow-2xl border border-white/20">
                ...
                {file ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">Supports: {Object.values(supportedFormats).flat().join(', ')}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Format Selection */}
            <div className="mb-6">
              <label className="block text-white font-semibold mb-3">Target Format</label>
                        ...
                        Object.entries(filteredFormats).map(([category, formats]) => (
                          <div key={category} className="p-4">
                            <h4 className="text-white font-semibold mb-3 capitalize flex items-center">
                              {category === 'audio' && <Music className="w-4 h-4 mr-2" />}
                              {category === 'video' && <Video className="w-4 h-4 mr-2" />}
                              {category === 'image' && <Image className="w-4 h-4 mr-2" />}
                              {category === 'document' && <FileText className="w-4 h-4 mr-2" />}
                              {category}
                            </h4>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                              {formats.map(format => (
                                ...
                                >
                                  {format.toUpperCase()}
                                  {target === format && <Check className="w-4 h-4" />}
                                </button>
                              ))}
                            </div>
                          </div>
            ...
            <div className="flex gap-4 mb-6">
              <button
                ...
                className={`flex-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 flex items-center justify-center space-x-2 ${
                  isConverting || !target || !file ? 'opacity-70 cursor-not-allowed' : 'hover:from-purple-600 hover:to-pink-600'
                }`}
              >
                {isConverting ? (
                  <>
            ...
            {message.text && (
              <div className={`p-4 rounded-xl mb-6 flex items-start space-x-3 ${
                ...
                {message.type === 'info' && <Info className="w-5 h-5 mt-0.5 flex-shrink-0" />}
                <span className="flex-1">{message.text}</span>
                <button onClick={() => setMessage({ text: '', type: '' })} className="text-white/50 hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
            )}
            ...
            {downloadUrl && (
              <div className="bg-green-500/20 border border-green-500/50 rounded-xl p-6 text-center">
                <h3 className="text-white font-semibold text-lg mb-4">Your file is ready! 🎉</h3>
                <a
                  href={downloadUrl}
                ...
                {recentConversions.map((conversion, index) => (
                  <div key={index} className="bg-white/5 rounded-lg p-4 border border-white/10">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="text-white font-medium truncate">{conversion.originalName}</p>
                        <p className="text-gray-400 text-xs">
                          {conversion.originalType} → {conversion.targetType}
                        </p>
                      </div>
                      <span className="text-xs text-gray-500">{conversion.date}</span>
                    </div>
                    <a
